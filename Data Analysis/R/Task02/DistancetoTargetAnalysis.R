...
# Read the CSV file
df <- read_csv("C:\\Users\\Brian\\Desktop\\Thesis Data\\Test\\Data Long Format\\Distance.csv")

df$Angles <- factor(df$Angles)

# Fit the ARTool model
m <- art(Distance ~ Angles + (1|PID), data = df)
...
means <- means %>%
  mutate(
    ymin = pmax(mean_distance - sd_distance, 0.06),
    ymax = mean_distance + sd_distance
  )

# Plot the data
ggplot(means, aes(x = Angles, y = mean_distance, fill = Angles)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.7) +
  geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, position = position_dodge(0.7)) +
  geom_text(aes(label = round(mean_distance, 1)), vjust = -0.5, position = position_nudge(x = 0.06)) +
