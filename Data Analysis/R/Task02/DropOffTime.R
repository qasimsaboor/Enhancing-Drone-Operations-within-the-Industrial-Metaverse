...
# Read the CSV file
df <- read_csv("C:\\Users\\Brian\\Desktop\\Thesis Data\\Test\\Data Long Format\\DropOffTime.csv")

df$Angles <- factor(df$Angles)

# Fit the ARTool model
m <- art(DropOffTime ~ Angles + (1|PID), data = df)
...
means <- df %>% 
  group_by(Angles) %>% 
  summarise(mean_droppofftime = mean(DropOffTime), sd_dropofftime = sd(DropOffTime))

# Ensure the lower bounds of the error bars are not less than zero
means <- means %>%
  mutate(
    ymin = pmax(mean_droppofftime - sd_dropofftime, 0.06),
    ymax = mean_droppofftime + sd_dropofftime
  )

# Plot the data
ggplot(means, aes(x = Angles, y = mean_droppofftime, fill = Angles)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.7) +
  geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, position = position_dodge(0.7)) +
  geom_text(aes(label = round(mean_droppofftime, 1)), vjust = -0.5, position = position_nudge(x = 0.06)) +
